...
class CI():
    ...
    def fit(self,
        ...
        for model in self.models:
            ...
            self.mean[model] = {grouped[self.region_col].iloc[i]: grouped['mean'].iloc[i]
                                for i in range(grouped.shape[0])}
            self.std[model] = {grouped[self.region_col].iloc[i]: grouped['std'].iloc[i]
                               for i in range(grouped.shape[0])}

            {grouped[self.region_col].iloc[i]: norm.interval(
                self.ci_range, loc=grouped['mean'].iloc[i], scale=grouped['std'].iloc[i]) for i in range(grouped.shape[0])}

            self.ci[model] = {grouped[self.region_col].iloc[i]: norm.interval(
                self.ci_range, loc=grouped['mean'].iloc[i], scale=grouped['std'].iloc[i]) for i in range(grouped.shape[0])}
                ...
                try:
                    if how == 'random':
                        if n_samples == 1:
                            seed = random_seed
                            while True:
                                spl = (norm.rvs(loc=self.mean[model][region],
                                                scale=self.std[model][region], random_state=seed)+1)
