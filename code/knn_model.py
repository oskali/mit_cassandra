...
class KNNModel():
    ...
    def fit(self, df , regions, clusters_map={}):
        self.state_df = df
        ...
        if self.region == 'county':
            
            self.state_df[self.region] = self.state_df['state'] + '-' + self.state_df[self.region] #renaming counties as State_county to avoid doublicate names
            ...
            remove_states_list = ['West Virginia','District of Columbia','Puerto Rico','American Samoa', 'Diamond Princess',
        ...
        mask = self.state_df[self.target] >= min_case_requirement
        ...
        self.startdates = {region : mod_date(min_dates.loc[region][self.date],15) for region in self.state_df[self.region].unique()}
    ...
    def predict(self, regions, dates, clusters_map={}):
