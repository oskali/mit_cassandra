...
class KNNModel():
    ...
    def fit(self, df, regions):
        ...
        self.startdates = {region: mod_date(min_dates.loc[region][self.date], 15)
                           for region in self.state_df[self.region].unique()}

        if self.use_mob:
            for state in self.state_df[self.region].unique():
                arr = self.state_df.loc[self.state_df[self.region] == state, self.mob_feature]
                self.state_df.loc[self.state_df[self.region] == state, self.mob_feature] = (
                    arr-np.nanmin(arr))/(np.nanmax(arr) - np.nanmin(arr))
