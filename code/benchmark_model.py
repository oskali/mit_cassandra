...
class SMLBenchMarkModel:
    ...
    def fit(self, data, scaling=True, save=True):
        ...
        if save:
            self.models_dict = {key: value for key, value in clf.model_dict.items(
            ) if key in {'Ridge', 'RandomForestRegressor', 'XGBRegressor'}}
        ...
        for name, model in tqdm(self.models_dict.items()):
            ...
            for region, row in self.X_val.reset_index().groupby(self.region_colname).first().iterrows():
                prediction_region = pd.DataFrame(
                    index=pd.date_range(start=row[0] + timedelta(1),
                                        periods=self.prediction_size, freq='1D', name=self.date_colname),
