...
def get_MDP(df_new, actions, pfeatures, n_cluster, complete=False, OutputFlag=0):
    ...
    transition_df = df_new.dropna(subset=['NEXT_CLUSTER']).groupby(
        ['CLUSTER', 'ACTION', 'NEXT_CLUSTER'])['RISK'].count()
...
def R2_value_testing(df_test, df_new, model, pfeatures, actions, n_cluster, OutputFlag=0):
    ...
    for id_cluster in df2.index:
        try:
            ...
            start_date, start_cluster, action = df_new.loc[df_new.ID ==
                                                           id_cluster, ['TIME', 'CLUSTER', 'ACTION']].tail(1).values[0]
...
def plot_path(df_new, df, state, h, pfeatures, plot=True, OutputFlag=0):
    ...
    while t < H:
        try:
            ...
            if OutputFlag >= 2:
                print('WARNING: Trying to predict next state from state', s, 'taking action',
                      a, ', but this transition is never seen in the data. Data point:', i, t)
        #a = df_test['ACTION'].loc[index + t]
        v_estim.append(math.exp(R_df.loc[s]))
        t += 1
...
def plot_pred_action(model, state, df_true, n_days, action_day=0, from_first=False):
    ...
    for a in actions:
        ...
        target_pred_act.plot(label='Predicted '+model.target_colname +
                             ' with ACTION {} after {} days'.format(a, action_day), ax=ax)
