...
def wrapper(df_orig,
    ...
    if run_knn:
        knn_output, _ = knn_fit_predict(
            ...
            day_0 = day_0, real_GR=True, deterministic=deterministic, target=target)
    ...
    for i in range(len(df)):
        val = 0
        if df.state[i] in weights.keys():
            if run_sir:
                val += weights[df.state[i]][0]*df.sir_prediction[i]
            if run_knn:
                weights[df.state[i]][1]*df.knn_prediction[i]
            if run_mdp:
                weights[df.state[i]][2]*df.mdp_prediction[i]
            preds.append(val)
        else:
            if run_knn:
                preds.append(df.knn_prediction[i])
            elif run_mdp:
                preds.append(df.mdp_prediction[i])
            else:
                preds.append(df.sir_prediction[i])
        if not(run_mdp):
            df['mdp_prediction'] = 0
        if not(run_knn):
            df['knn_prediction'] = 0
