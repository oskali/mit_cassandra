...
for zipcode_number in zip_codes:
    ...
    total_population = sum(
        [search.by_zipcode(z).population for z in zip_list if search.by_zipcode(z).population is not None])
...
if load_ci:
    ...
    for model_type in sampled_output.keys():
        ...
        samples['samples'] = [[{state: samples['samples'][i][state][date] for state in samples['samples']
                                [0].keys()} for date in range(len(samples['dates']))] for i in range(n_samples)]
        ...
        if new_cases:
            ...
            #new_samples['samples'] = []
            tags = ['Total_uncontained_infections', 'Total_contained_infections', 'Total_confirmed_infections',
                    'New_uncontained_infections', 'New_contained_infections', 'New_confirmed_infections']

            l = []
            for i in range(len(samples['samples'])):
                cl_dic = {}
                for state in samples['samples'][i][0].keys():
                    sub_sample = [samples['samples'][i][j][state] - samples['samples'][i][j-1][state]
                                  for j in range(1, len(dates))]
                    total_new = [sum(sub_sample[i+j] for i in range(infection_period))
                                 for j in range(len(dates)-infection_period)]
                    ...
                    confirmed_sub_sample = [confirmed_samples['samples'][i][j][state] -
                                            samples['samples'][i][j-1][state] for j in range(1, len(dates))]
                    confirmed_total_new = [sum(confirmed_sub_sample[i+j] for i in range(infection_period))
                                           for j in range(len(dates)-infection_period)]

                    confirmed_sub_sample_total = [confirmed_samples['samples'][i][j][state]
                                                  for j in range(1, len(dates))]
                    confirmed_total = [sum(confirmed_sub_sample_total[i+j] for i in range(infection_period))
                                       for j in range(len(dates)-infection_period)]

                    cl_dic[state] = (total_severe, total_mild, confirmed_total,
                                     total_new_severe, total_new_mild, confirmed_total_new)
                sample_dic = [{state: {tags[i]: cl_dic[state][i][j] for i in range(
                    len(tags))} for state in samples['samples'][i][0].keys()} for j in range(len(dates)-infection_period)]
                if use_zips:
                    sample_dic = [{zipcode_number: {tags[i]: cl_dic[regions_dic[zipcode_number]][i][j]*ratio_dic[zipcode_number]
                                                    for i in range(len(tags))} for zipcode_number in zip_codes} for j in range(len(dates)-infection_period)]
                l.append(sample_dic)
