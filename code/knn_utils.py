...
def transpose_case_df(simple_output, forward_days, day_0, date_col='date', region_col='county', target_col='cases'):
    ...
    # , 'pred_cases_low':low_cases, 'pred_cases_high': high_cases})
    df = pd.DataFrame({date_col: dates, region_col: states, 'pred_' + target_col: cases})
    return df


def get_best_parameters(df, chosen_region, memory, split_date, forward_days, r, day1, region_col='state', col_date='date', mob_col='mob', use_mob=False, starting_point='high', hfilter=False, clusters_map={}):
    ...
    for threshold in [100]:  # 1.1, 1.5, 2, 5, 10, 20, 50,
        for n in [1, 2, 3, 5, 8, 10, 15, 20, 30, 50, 100]:
            for p in [1, 2]:
                ...
                train = get_in_date_range(df, first_date=day1, last_date=mod_date(split_date, -r), date_col=col_date)
                test = get_in_date_range(df, first_date=mod_date(split_date, -r),
                                         last_date=split_date, date_col=col_date)
                ...
                if (test.shape[0] != 0 and train.shape[0] != 0):
                    if (n <= train.shape[0]):
                        ...
                        modelsq = KNeighborsRegressor(
                            n_neighbors=n, weights=lambda x: get_weights_sq(x, threshold), p=p)

                        # fit is not actually training anything, just assigns each y value to corresponding x vector
...
def match_to_real_growth(df, region_best_parameters, list_states, start_date, memory, forward_days, day_0, day1, split_date, deterministic, region_col='county', date_col='date', mob_col='mob', use_mob=False, starting_point='high', hfilter=False, clusters_map={}):
    ...
    feature_choices = ['GrowthRate_t-' + str(i) for i in [memory-i for i in range(memory)]] + \
        ['pred_forward_day_' + str(i) for i in range(forward_days)]
    ...
    for i in range(forward_days):
        ...
        real_features = ['GrowthRate_t-' + str(j+1) for j in range(memory)]  # + ['transit_stations_smooth_shift']
        ...
        for chosen_region in list_states:
            ...
            train_data_in_state = get_in_date_range(
                df.loc[df[region_col] == chosen_region], first_date=start_date[chosen_region], last_date=day_0, date_col=date_col)
            # for out of state train data, we can use anything before the split_date'
            train_data_out_of_state = (get_in_date_range(
                df.loc[df[region_col] != chosen_region], first_date=day1, last_date=split_date, date_col=date_col))
            ...
            test0 = get_in_date_range(test_df, first_date=mod_date(
                day_0, 0), last_date=mod_date(day_0, 1), date_col=date_col)
            ...
            if deterministic:
                if use_mob:
                    # scale remaining weights so they add to 1
                    scaled_weights = selected_weights * 1/sum(selected_weights)
...
def cassandra_fit(df, list_states, start_date, memory=10, forward_days=14, split_date='2020-05-01', day_0='2020-05-01', day1='2020-04-10', real_GR=False, deterministic=True, r=1, date_col='date', region_col='state', target_col='cases', mob_col='mob', use_mob=False, starting_point='high', hfilter=False, clusters_map={}):
    '''
    ...
    df = df.loc[~df[region_col].isin(['West Virginia', 'District of Columbia', 'Puerto Rico', 'American Samoa',
                                      'Diamond Princess', 'Grand Princess', 'Guam', 'Northern Mariana Islands', 'Virgin Islands'])]
    df = df.sort_values(by=[region_col, date_col])  # has to be sorted by days to create growth rates

    # group by state so that consecutive rows are consecutive days in a single state
    df['GrowthRate'] = df.groupby(region_col)[target_col].shift(0) / (df[target_col].shift(1)) - 1
    ...
    for region in list_states:
        threshold, n, p, func = get_best_parameters(df, region,  memory, split_date, forward_days, r, day1, region_col=region_col,
                                                    col_date=date_col, mob_col=mob_col, use_mob=use_mob, starting_point=starting_point, hfilter=hfilter, clusters_map=clusters_map)
        region_best_parameters[region] = {'threshold': threshold, 'neighbors': n, 'p_norm': p, 'function': func}

    return region_best_parameters


def cassandra_predict(df, region_best_parameters, list_states, start_date, memory=10, forward_days=14, split_date='2020-05-01', day_0='2020-05-01', day1='2020-04-10', real_GR=False, deterministic=True, r=1, date_col='date', region_col='state', target_col='cases', mob_col='mob', use_mob=False, starting_point='high', hfilter=False, clusters_map={}):
    ...
    '''
    predictions = match_to_real_growth(df, region_best_parameters, list_states, start_date, memory, forward_days, day_0, split_date,
                                       day1, deterministic, region_col=region_col, mob_col=mob_col, use_mob=use_mob, starting_point=starting_point, hfilter=hfilter)
    ...
    for i in range(1, forward_days):
        predictions['pred_growth_for_next_{}days'.format(
            i+1)] = predictions['pred_growth_for_next_{}days'.format(i)]*(predictions['pred_forward_day_' + str(i)] + 1)
        predictions['pred_high_growth_for_next_{}days'.format(
            i+1)] = predictions['pred_growth_for_next_{}days'.format(i)]*(predictions['pred_high_day_' + str(i)] + 1)
        predictions['pred_low_growth_for_next_{}days'.format(
            i+1)] = predictions['pred_growth_for_next_{}days'.format(i)]*(predictions['pred_low_day_' + str(i)] + 1)
    for i in range(forward_days):
        predictions['pred_growth_for_next_{}days'.format(
            i+1)] = predictions['pred_growth_for_next_{}days'.format(i+1)] - 1
        predictions['pred_high_growth_for_next_{}days'.format(
            i+1)] = predictions['pred_high_growth_for_next_{}days'.format(i+1)] - 1
        predictions['pred_low_growth_for_next_{}days'.format(
            i+1)] = predictions['pred_low_growth_for_next_{}days'.format(i+1)] - 1

    # convert cumulative growth rates to cases
    for i in range(forward_days):
        predictions[target_col + '_predicted_day_' +
                    str(i)] = np.round(predictions[target_col+'_t-1']*(predictions['pred_growth_for_next_{}days'.format(i+1)]+1))
        predictions[target_col+'_high_predicted_day_' + str(i)] = np.round(
            predictions[target_col+'_t-1']*(predictions['pred_high_growth_for_next_{}days'.format(i+1)]+1))
        predictions[target_col+'_low_predicted_day_' + str(i)] = np.round(
            predictions[target_col+'_t-1']*(predictions['pred_low_growth_for_next_{}days'.format(i+1)]+1))

    columns_to_keep = [region_col, date_col, target_col] + [target_col+'_predicted_day_' + str(i) for i in range(forward_days)] + [
        target_col+'_low_predicted_day_' + str(i) for i in range(forward_days)] + [target_col+'_high_predicted_day_' + str(i) for i in range(forward_days)]
