...
def time_clustering(state_df, day_0, days_before,print_knee = False):
    ...
    clusters_map : dictionairy of state : state's cluster
    """
    
    cluster_state_df = state_df.copy()
    cluster_state_df['GrowthRate'] = (state_df.groupby('state')['cases'].shift(0) / state_df['cases'].shift(1) - 1)
    
    cluster_state_df = get_in_date_range(cluster_state_df,
                                         first_date = mod_date(day_0, -days_before),
                                         last_date = mod_date(day_0, 0))
    
    cluster_state_df = cluster_state_df.loc[:, cluster_state_df.columns.intersection(['state','Date','GrowthRate'])]
    
    time_series = cluster_state_df.groupby('state')['GrowthRate'].apply(list)
    ...
    kn = KneeLocator(range(1,40,1), inertias,curve='convex', direction='decreasing')
    
    # the next 5 lines can be removed
    if print_knee:
        print("Optimal value of clusters is: ", kn.knee)
        
        plt.xlabel('number of clusters k')
        plt.ylabel('Sum of squared distances')
        plt.plot(range(1,40,1), inertias, 'bx-')
        plt.vlines(kn.knee, plt.ylim()[0], plt.ylim()[1], linestyles='dashed')
    
    model = TimeSeriesKMeans(n_clusters=kn.knee, metric="dtw", max_iter=100, dtw_inertia=True, n_jobs=-1)
    model.fit(time_series_list)
    
    states = cluster_state_df['state'].unique().tolist()
    
    clusters = [[] for _ in range(0,kn.knee,1)]

    for i in range(len(model.labels_)):
        clusters[model.labels_[i]].append(states[i])
        
    clusters_map = {} # dictionairy that maps each state to its cluster of states
    for i in clusters:
        for j in i:
            clusters_map[j] = i
...
def cassandra_fit(df, list_states, start_date, memory = 10, forward_days = 14, split_date = '2020-05-01', day_0 = '2020-05-01', day1='2020-04-10', real_GR = False, deterministic = True, r = 1, date_col='date', region_col='state', target_col='cases', mob_col = 'mob', use_mob = False, starting_point = 'high', hfilter = False , clusters_map= {} ):
    ...
    df = df.loc[~df[region_col].isin(['West Virginia','District of Columbia','Puerto Rico','American Samoa', 'Diamond Princess','Grand Princess','Guam','Northern Mariana Islands','Virgin Islands'])]
    ...
    for region in list_states:
        threshold, n, p, func = get_best_parameters(df, region ,  memory, split_date, forward_days, r, day1, region_col= region_col, col_date=date_col, mob_col = mob_col, use_mob = use_mob, starting_point = starting_point, hfilter = hfilter , clusters_map= clusters_map)
...
def cassandra_predict(df, region_best_parameters, list_states, start_date, memory = 10, forward_days = 14, split_date = '2020-05-01', day_0 = '2020-05-01', day1='2020-04-10', real_GR = False, deterministic = True, r = 1, date_col='date', region_col='state', target_col='cases', mob_col = 'mob', use_mob = False, starting_point = 'high', hfilter = False, clusters_map= {} ):
    ...
    '''    
    predictions = match_to_real_growth(df, region_best_parameters, list_states, start_date, memory, forward_days, day_0, split_date, day1, deterministic, region_col=region_col, mob_col = mob_col, use_mob = use_mob, starting_point = starting_point, hfilter = hfilter)
