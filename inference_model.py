...
from models.prevalence.perakis_prevalence.data_utils import (load_model)

from models.common.model import Model

from models.prevalence.perakis_prevalence import knn_utils, mdp_testing, data_utils, bilstm_utils, params
sys.modules['knn_utils'] = knn_utils
sys.modules['mdp_testing'] = mdp_testing
sys.modules['data_utils'] = data_utils
sys.modules['bilstm_utils'] = bilstm_utils
sys.modules['params'] = params
from models.prevalence.perakis_prevalence import mdp_utils
sys.modules['mdp_utils'] = mdp_utils
from models.prevalence.perakis_prevalence import mdp_states_functions
sys.modules['mdp_states_functions'] = mdp_states_functions

from models.prevalence.perakis_prevalence import sir_model, knn_model, mdp_model, agg_model, bilstm_model, confidence_intervals, prevalence
sys.modules['sir_model'] = sir_model
sys.modules['knn_model'] = knn_model
sys.modules['agg_model'] = agg_model
sys.modules['mdp_model'] = mdp_model
sys.modules['bilstm_model'] = bilstm_model
sys.modules['confidence_intervals'] = confidence_intervals
sys.modules['prevalence'] = prevalence
...
class InferenceModel(Model):
    def sample(self,  t_0: str, n_samples: int, dates: list, input_samples: dict) -> dict:
        ...
        fips = pd.read_csv(os.path.join(
            dir_path, self.model_parameters['fips_path']))


        sir = load_model(os.path.join(
            dir_path, self.model_parameters['sir_file']))

        
        knn = load_model(os.path.join(
            dir_path, self.model_parameters['knn_file']))

       
        mdp = load_model(os.path.join(
            dir_path, self.model_parameters['mdp_file']))

        # bilstm_file = pd.read_csv(os.path.join(
        #     repo_path, self.model_parameters['bilstm_file']))
        bilstm = load_model(os.path.join(
            dir_path, self.model_parameters['bilstm_file']))

        # agg_file = pd.read_csv(os.path.join(
        #     repo_path, self.model_parameters['agg_file']))
        agg = load_model(os.path.join(
            dir_path, self.model_parameters['agg_file']))

        # ci_file = pd.read_csv(os.path.join(
        #     repo_path, self.model_parameters['ci_file']))
        ci = load_model(os.path.join(
            dir_path, self.model_parameters['ci_file']))

        # preval_file = pd.read_csv(os.path.join(
        #     repo_path, self.model_parameters['preval_file']))
        preval = load_model(os.path.join(
            dir_path, self.model_parameters['preval_file']))
        ...
        #new_samples['samples'] = []
        #print('Line 178 samples:')
        #print(new_samples['dates'])
