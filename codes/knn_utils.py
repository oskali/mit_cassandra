...
def add_one_fake_day_forward(original_df, date = 'date', region = 'state'):
    temp = original_df.loc[original_df[date] == max( original_df[date] )]
    temp[date] = temp.apply(lambda row: mod_date(row[date], 1), axis=1)
    return pd.concat([original_df, temp]).sort_values(by=[region , date])
...
def get_best_parameters(df, chosen_region , memory, split_date, forward_days, r, day1, region_col = 'state',
        ...
        if extra_features:
            ...
            I = 20 # number of random sampling of weights
        ...
        for sample_iteration in range(I):
            for threshold in [100]: ##,1.5, 2, 5, 10, 20, 50, 100
...
def match_to_real_growth(df, region_best_parameters, list_states, start_date, memory, forward_days, day_0, day1,
    ...
    for i in tqdm(range(forward_days)):
...
def cassandra_fit(df, list_states, start_date, memory = 10, forward_days = 14, split_date = '2020-05-01', day_0 = '2020-05-01',
    ...
    for region in tqdm(list_states):
