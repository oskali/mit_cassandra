...
class MDP_Splitter:
    ...
    def update_mdp(self):
        ...
        self.P_df, self.R_df = get_MDP(self.df)
    ...
    def compute_risk_error(self):
        ...
        for h in range(1, self.horizon+1):  # [TO ADAPT WITH EXPONENTIAL]
            ...
            self.df["EST_H_NEXT_CLUSTER"] = self.df.groupby(['ID']).apply(lambda group: group.ffill())['EST_H_NEXT_CLUSTER'].values
            ...
            self.df["TRUE_H_NEXT_RISK"] = self.df.groupby("ID").rolling(window=h)["RISK"].sum().values
            self.df["TRUE_H_NEXT_RISK"] = self.df.groupby("ID")["TRUE_H_NEXT_RISK"].shift(-h)
...
def contradiction(df,  # pandas dataFrame
    ...
    if len(nc) <= 1:
        ...
        nc = nc.groupby("NEXT_CLUSTER")["EST_H_ERROR"].sum().argmax()
        return a, nc
...
def splitter(splitter_dataframe,  # pandas dataFrame
    ...
    testing_error = []
    training_error = []
    k = splitter_dataframe.df['CLUSTER'].nunique() #initial number of clusters
    nc = k
    # df_new = deepcopy(df)
    ...
    for _ in split_bar:
        ...
        if testing:
            ...
            splitter_dataframe.update_cluster()
            df_test["CLUSTER"] = splitter_dataframe.model.predict(df_test.iloc[:, 2:splitter_dataframe.pfeatures+2])
            df_test["NEXT_CLUSTER"] = df_test.groupby("ID")["CLUSTER"].shift(-1)

            # instantiate testing
            test_splitter_dataframe = MDP_Splitter(df_test,
                                                   pfeatures=splitter_dataframe.pfeatures,
                                                   clustering=splitter_dataframe.clustering,
                                                   init_n_clusters=splitter_dataframe.init_n_clusters,
                                                   distance_threshold=splitter_dataframe.distance_threshold,
                                                   random_state=splitter_dataframe.random_state)
            ...
            test_error = testing_value_error(test_splitter_dataframe, periods=5)
        ...
        if c != -1:
            ...
            # if OutputFlag == 1:
            splitter_dataframe.df = split(splitter_dataframe.df, c, a, b, pfeatures, nc, classification, random_state=random_state).copy()
