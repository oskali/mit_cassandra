...
class BILSTMModel():
    ...
    def fit(self,
        ...
        self.state_df = self.state_df[self.state_df[self.target] >= limit]
        state_df = self.state_df.copy()
        state_df = state_df[state_df['temperature'].notna()]
        # state_df = state_df[state_df['growthcasespred'].notna()]
        state_df = state_df[[self.region, self.date,self.target, 'temperature']].groupby([self.region, self.date]).sum().reset_index()
        ...
        clusters = [list(state_df[self.region].unique())]
        ...
        # df0['growthcases_lag'] = df0.groupby(self.region)['growthcasespred'].shift(20)
        df0['temperature_lag'] = df0.groupby(self.region)['temperature'].shift(5)
        ...
        for i in range(self.memory):
            ...
            df0['temperature_t-' + str(i+1)] = df0.groupby(self.region)['temperature_lag'].shift(i+1)
            # df0['growthcaseslag_t-' + str(i+1)] = df0.groupby(self.region)['growthcases_lag'].shift(i+1)
        ...
        for c in self.clusters:
            ...
            (model_winner,mode_winner,epochs_winner, auto_epochs) = find_best_model_covid(df = self.state_df, start_date = startdates,
                                                            cluster = c, use_auto = self.use_auto, memory = self.memory, 
                                                            split_date = split_date, day_0 = day_0, 
                                                            real_GR = True, deterministic = deterministic,
                                                            r = 26,  date_col=self.date,
                                                            region_col=self.region,
                                                            target_col=self.target)

            # (model_winner,mode_winner,epochs_winner, auto_epochs) = ('model_1','ave',1,1)
            ...
            _ = fit_covid(df = self.state_df, start_date = startdates, 
                                ...
                                memory = self.memory, day_0 = day_0, split_date = split_date,
    ...
    def predict(self, regions, dates, argument = ''):
        ...
        for i in range(forward_days):
            ...
            self.state_df['temperature_t+' + str(i)] = self.state_df.groupby(self.region)['temperature_lag'].shift(-i)
            # self.state_df['growthcaseslag_t+' + str(i)] = self.state_df.groupby(self.region)['growthcases_lag'].shift(-i)


        df_simple = pd.DataFrame()
        for c in self.clusters:
            cluster_index =self.clusters.index(c)
            self.model = load_model(model_path+"/bidir_lstm_cluster_"+str(cluster_index)+"_"+argument+".h5")
            if (self.use_auto):
                self.auto_model = load_model(model_path+"/auto_bidir_lstm_cluster_"+str(cluster_index)+"_"+argument+".h5")
            print(self.clusters)
            df_simple_temp = predict_covid(df = self.state_df, start_date = self.startdates, 
                                                            ...
                                                            memory = self.memory, forward_days = forward_days, 
                                                            split_date = split_date, day_0 = day_0, 
