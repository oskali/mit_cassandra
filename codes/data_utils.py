...
from params import (target_col, date_col, region_col, training_cutoff,
                         df_path, default_path, nmin, restriction_dict, region_exceptions_dict, validation_cutoff)
import os
import warnings
...
def load_data(file=df_path,
    ...
    try:
        df.loc[df.state == "Louisiana", "popdensity"] = 104.9
        df.loc[df.state == "Louisiana", "commute_90_more_mins"] = 1910
    except:
        pass

    try:
        df["cases_nom"] = df["cases"] * df["popdensity"] / df["population"]
        df["deaths_nom"] = df["deaths"] * df["popdensity"] / df["population"]
    except KeyError:
        pass
    ...
    df["cases_pct30"] = df.groupby(region)["cases"].pct_change(30).values
    df["cases_pct40"] = df.groupby(region)["cases"].pct_change(40).values

    # percentaage change in cases
    df["cases_pct3_lag7"] = df.groupby(region)["cases_pct3"].shift(7).values
    df["cases_pct5_lag7"] = df.groupby(region)["cases_pct5"].shift(7).values
    df["cases_pct3_lag14"] = df.groupby(region)["cases_pct3"].shift(14).values
    df["cases_pct5_lag14"] = df.groupby(region)["cases_pct5"].shift(14).values
    df["cases_pct3_lag30"] = df.groupby(region)["cases_pct3"].shift(30).values
    df["cases_pct5_lag30"] = df.groupby(region)["cases_pct5"].shift(30).values
    df["cases_pct3_lag60"] = df.groupby(region)["cases_pct3"].shift(60).values
    df["cases_pct5_lag60"] = df.groupby(region)["cases_pct5"].shift(60).values
    ...
    df["temperature_7"] = df.groupby(region)["temperature"].shift(7).values
    df["temperature_3"] = df.groupby(region)["temperature"].shift(3).values
    df["prcp_median_scaled_5"] = df.groupby(region)["prcp_median_scaled"].shift(5).values
    ...
    #     df["grocery_and_pharmacy_1w"] = df.groupby(region)["grocery_and_pharmacy"].diff(28).fillna(method="ffill").values
    ...
    #     # normalized by pop density
    #     df["in_school_pct_popdensity"] = df["in_school"] * df["popdensity"] / df["population"]
    #     df["civilian_labor_force_pct_popdensity"] = df["civilian_labor_force"] * df["popdensity"] / df["population"]
    #     df["pop_determined_poverty_status_pct_popdensity"] = df["pop_determined_poverty_status"] * df["popdensity"] / df["population"]

    #     df["black_pop_pct_popdensity"] = df["black_pop"] * df["popdensity"] / df["population"]
    #     df["white_pop_pct_popdensity"] = df["white_pop"] * df["popdensity"] / df["population"]
    #     df["asian_pop_pct_popdensity"] = df["asian_pop"] * df["popdensity"] / df["population"]
    #     df["hispanic_pop_pct_popdensity"] = df["hispanic_pop"] * df["popdensity"] / df["population"]
    #     df["not_hispanic_pop_pct_popdensity"] = df["not_hispanic_pop"] * df["popdensity"] / df["population"]
    #     df["not_us_citizen_pop_pct_popdensity"] = df["not_us_citizen_pop"] * df["popdensity"] / df["population"]
    #     df["other_race_pop_pct_popdensity"] = df["other_race_pop"] * df["popdensity"] / df["population"]

    #     # normalized by white population
    #     df["black_pop_pct_white_pop"] = df["black_pop"] / df["white_pop"]
    #     df["asian_pop_pct_white_pop"] = df["asian_pop"] / df["white_pop"]
    #     df["hispanic_pop_pct_white_pop"] = df["hispanic_pop"] / df["white_pop"]
    #     df["not_hispanic_pop_pct_white_pop"] = df["not_hispanic_pop"] / df["white_pop"]
    #     df["not_us_citizen_pop_pct_white_pop"] = df["not_us_citizen_pop"] / df["white_pop"]
    #     df["other_race_pop_pct_white_pop"] = df["other_race_pop"] / df["white_pop"]
...
def get_last_day_mapes(df,
              models,
              region_col='state',
              target_col='cases'):

    df = df.loc[df['date']==validation_cutoff]
    results = []
    for region in set(df[region_col]):
        df_sub = df[df[region_col] == region]
        results.append([region] + [mape(df_sub[target_col], df_sub[model]) for model in models])
