...
df_agg = dict_to_df(output,
                    df_test)
df_agg.to_csv("../results/backtest_tr_cutoff_{}_{}.csv".format(training_cutoff, validation_cutoff))

results = get_mapes(df_agg.dropna(subset=["agg"]),
                    models,
                    region_col=region_col,
                    target_col=target_col)
results.to_csv("../results/mapes_{}_{}.csv".format(training_cutoff, validation_cutoff))

export = df.merge(df_agg.iloc[:, :-1], how='left', on=[region_col, date_col])
export.to_csv("../results/export_{}_{}.csv".format(training_cutoff, validation_cutoff))
