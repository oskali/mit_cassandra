...
def model(ini, time_step, params):
    ...
    Y[0] = 0 - (beta*S*I)/sum(ini) #S
    Y[1] = (beta*S*I)/sum(ini) - a*E #E
...
def inifcn(params, cases, deaths, pop_est, iniest = None):
    if(iniest is None):
        S0 = pop_est - cases[0] - deaths[0]
        E0 = 0.0
        I0 = cases[0]
        R0 = 0.0
        D0 = deaths[0]
...
def NLL(params, pop_est, cases, deaths, recover, death_lm, recover_lm, weight_dead, weight_recover, times, ini): #loss function
    params = np.abs(params)
    cases = np.array(cases)
    deaths = np.array(deaths)
    res = ode(model, inifcn(params,cases,deaths, pop_est, ini), times, args =(params,))
...
class SIRModel():
            ...
            for i in range(len(regions)):
                ...
                if train_full_set.shape[0] > self.nmin_train_set and region_pop > 0:
                    ...
                    for w in range(len(wave_list)):
                        ...
                        S_t = np.array(region_pop) - wave_cum_cases

                        beta = sum(i_t)/sum(wave_cases*S_t/region_pop)
                            ...
                            for gammaindex in gamma_progress:
                                for aindex in a_progress:
                                    for muindex in mu_progress:
                                        ...
                                        params = [region_betavals[betaindex], region_gammavals[gammaindex], region_avals[aindex], region_muvals[muindex]]
                                        if w==0:
                                            iniests = inifcn(params, wave_cases, wave_dead, region_pop)
                                        else:
                                            iniests = prev_xest[len(prev_xest)-1]
                                        optimizer = optimize.minimize(NLL, params, args=(region_pop, wave_cases, wave_dead, wave_recover, 2,2, weight_dead, weight_recover, wave_times, iniests), method= 'Nelder-Mead')

                                        paramests = np.abs(optimizer.x)
                                        #print(paramests[0])
                                        if w == 0:
                                            iniests = inifcn(paramests, wave_cases, wave_dead, region_pop)
                                        else:
                                            iniests = prev_xest[len(prev_xest)-1]
                                        ...
                                        #mse = sqrt(mean_squared_error(wave_cases,active)) + death_lm*sqrt(mean_squared_error(wave_dead,dead)) + recover_lm*sqrt(mean_squared_error(wave_recover,recovered))
                        ...
                        # active, recovered, dead = finfcn(xest)
                        # plt.plot(wave_times, active, label = "est")
                        # plt.plot(wave_times, wave_cases, label = "true")
        ...
        def predict(self,
                    ...
                    dates):
            results = dict()
            regions_trained = [value for value in self.trained_param.keys() if value in regions]

            for i in range(len(regions_trained)):
                region = regions_trained[i]
