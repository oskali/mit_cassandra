...
@author: janiceyang, omars, amineb, davidnzendong
"""
...
from randMDPModel.rmdp_states_functions import createSamples, createSamplesRelative, fit_cv, fit_eval
from randMDPModel.rmdp_utils import MDP_Splitter, splitter, MDPPredictorError, process_prediction_region
from randMDPModel.rmdp_testing import predict_region_date, \
...
class rMDPModel:
    def __init__(self,
        ...
        self.randomized_split_pct = randomized_split_pct  # fraction of IDs to include in a randomized sampling
        ...
        self.nfeatures = nfeatures  # number of features (if randomized sampling)
        ...
        self.features = None  # list of the features
        self.pfeatures = None  # number of features
        self.actions = None  # 
        self.splitter_dict = {}

        # Calibration after the training (only for exponential mean)
    ...
    def __str__(self):
        ...
        return "rmdp__{}_{}_{}__h{}__davg{}__cdt_{}pct__n_iter{}".format(self.reward_name[0],
                                                                        self.region_colname[:4],
                                                                        self.target_colname,
                                                                        self.horizon,
                                                                        self.days_avg,
                                                                        int(self.clustering_distance_threshold * 100),
                                                                        self.n_iter)  # need to be change to be unique
    ...
    # - 'TIME_CV' (DEFAULT VAlUE) : the data are split onto folds  according to the ID columns,
    ...
    def fit(self,
        ...
        # find the best cluster (regular MDP version)
        try:
        ...
        if self.verbose >= 1:
            print('the optimal number of iterations:', k)
        ...
        # save the MDP corresponding to the optimal number of cluster (k-opt MDP)
        self.splitter_dict["k_opt"] = trained_splitter_df
        
        # save the random mdps
        for _ in self.splitter_dict.keys():
            if _ != "k_opt":
                self.splitter_dict[_] = (self.splitter_dict[_][0], self.splitter_dict[_][1][self.optimal_cluster_size - self.splitter_dict[_][1][0][0]][1])

        # store trained dataset and predict_cluster function
        self.df_trained = df
        self.df_trained = self.df_trained.groupby(self.region_colname).last()

        # store the first clusters (Dev)
        if self.keep_first:
            if self.verbose >= 2:
    ...
    # [DEV - Exponential mean filtering] (can be removed if not used)
    # calibrate an exponential filtering coefficient per ID : 
    #  corresponding weight w(x_i) to the prediction of MPD i, x_i : w(x_i) = exp(- alpha_ID * (x_i - ^E[x]) / ^sigma(x)))
    ...
    # predict() takes a state name and a time horizon, and returns the predicted value
...
# number of cases after h steps from the most recent datapoint
    def predict_region_ndays(self,
        ...
        try:
            if not (model_key in {"median", "best_r2", "best_err", "mean", "std", "std_diff", 'all',
                ...
                try:
                    if not agg:
                        mdp_predictor = self.splitter_dict[model_key][0]
                    else:
                        mdp_predictor = self.splitter_dict[model_key][-1]
            ...
            # best r2 score
                ...
                return self.predict_region_ndays(region, n_days, model_key=best_key, agg=agg, from_first=from_first)
    ...
    def predict_allregions_ndays(self,
                                 ...
                                 from_first=False,
                                 agg=False,
                                 model_key="k_opt"): # time horizon for prediction, preferably a multiple of days_avg (default 3)
        df = self.df_trained.copy()
        df = df[['TIME', "TARGET"]]
        df['TIME'] = df['TIME'] + timedelta(n_days)
        if self.reward_name == "RISK":
            df["TARGET"] = df.index.map(
                lambda region: np.ceil(self.predict_region_ndays(region, n_days, from_first=from_first, model_key=model_key, agg=agg)))
        elif self.reward_name == "AlphaRISK":
            df["TARGET"] = df.index.map(
                lambda region: np.ceil(self.predict_region_ndays_alpha(region, n_days, from_first=from_first, model_key=model_key, agg=agg)))
        return df
    ...
    def predict(self,
        ...
        if n_jobs in {0, 1}:
            for region in regions:
                ...
                pred_region_df = process_prediction_region(region, self, dates, region_set, agg=agg, model_key=model_key, from_first=from_first)
