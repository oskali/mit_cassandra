...
from randMDPModel.rmdp_utils import fit_cv_fold, fit_eval_params
...
def createSamplesRelative(df,  # dataframe: original full dataframe
                  # new_cols, # str list: names of columns to be considered
                  target_colname,  # str: col name of target_colname (i.e. 'deaths')
                  region_colname,  # str, col name of region (i.e. 'state')
                  date_colname,  # str, col name of time (i.e. 'date')
    ...
    # remove missing observations until the latest missing one
    df_new.loc[:, ['TIME']] = pd.to_datetime(df_new['TIME'])
    dfs = []
    for region_name, group_region in df_new.groupby(region_colname):
        first_date = group_region.TIME.min()
    ...
    # resample data according to # of days
    df_new[target_colname] = df_new.groupby(['ID']).rolling(min_periods=3, window=3)[target_colname].mean().values
    g = df_new.groupby(['ID'])
    cols = df_new.columns
    # print('cols', cols)
    dictio = {i: 'last' for i in cols}
    ...
    # alpha function
    df_new['alpha_t'] = (df_new['r_t'] + 1e-6) / (df_new['r_t-1'] + 1e-6) - 1.
    df_new['alpha_t'].replace([np.inf], np.e**(5.), inplace=True)
    df_new['alpha_t'].replace([0], np.e**(-5.), inplace=True)
    df_new['alpha_t-1'] = df_new.groupby("ID")['alpha_t'].shift(1)
    df_new['alpha_t-2'] = df_new.groupby("ID")['alpha_t'].shift(2)

    new_features = features_list+[f+'-1' for f in features_list] + [f+'-2' for f in features_list] + ['r_t', 'r_t-1', 'r_t-2'] + ['alpha_t', 'alpha_t-1', 'alpha_t-2']
    df_new.dropna(subset=new_features,
                  inplace=True)

    # Here we assign initial clustering by r_t
    df_new['RISK'] = np.log(df_new['r_t'] + 1e-6)
    df_new['RISK_NEXT'] = df_new.groupby("ID")["RISK"].shift(-1).values
    df_new['RISK_SCALE'] = np.tanh(df_new["RISK"] / 0.2)
    df_new['AlphaRISK'] = np.log(df_new["RISK"] / df_new.groupby("ID")["RISK"].shift(1).values)
    df_new['AlphaRISK_NEXT'] = df_new.groupby("ID")["AlphaRISK"].shift(-1).values
    df_new['AlphaRISK_SCALE'] = np.tanh(df_new["AlphaRISK"])
    df_new.replace([-np.inf], -5., inplace=True)
    df_new.replace([np.inf], 5., inplace=True)
    ...
    # Drop all rows with empty cells
    df_new.dropna(subset=['AlphaRISK'], inplace=True)
    df_new.rename(columns={target_colname: 'TARGET'}, inplace=True)

    pfeatures = len(df_new.columns)-11
...
def fit_cv(df,
    ...
    if cv in {0, 1} or mode == "ALL":
        split_ = [(0, (df.index, df.index), pfeatures)]
    ...
    # cross validation
        ...
        if not randomized:
            ids = list(set(df['ID']))
            random.shuffle(ids)
            map_id = {_: __ for _, __ in enumerate(ids)}
            df['ID_shuffle'] = df['ID'].map(map_id)
            features_to_include = [df.columns[2 + _] for _ in range(pfeatures)]
            split_ = [(idx, _, features_to_include) for idx, _ in enumerate(GroupKFold(n_splits=cv).split(df, y=None, groups=df['ID_shuffle']))]
        else:
            split_ = []
            if nfeatures is None:
                nfeatures = pfeatures
            elif isinstance(nfeatures, int):
                nfeatures = min(nfeatures, pfeatures)
            elif isinstance(nfeatures, float) & (nfeatures < 1.) & (nfeatures > 0.):
                nfeatures = min(round(pfeatures * nfeatures), pfeatures)
            else:
                pass
            ...
            for i in range(cv):
                ...
                map_id = {_: __ for _, __ in enumerate(ids)}
                df['ID_shuffle'] = df['ID'].map(map_id)
                cutoff = min(len(map_id), round(randomized_split_pct * len(map_id)))
                df_cutoff = df[df.ID_shuffle <= cutoff].reset_index()

                # features to include
                features_to_include = list(range(pfeatures))
                random.shuffle(features_to_include)
                features_to_include = features_to_include[:nfeatures]
                features_to_include = [df.columns[2 + _] for _ in features_to_include]

                # 4 is hard variable, it can be changed to 5 or 3
                for split_fold in GroupKFold(n_splits=4).split(df_cutoff, y=None, groups=df_cutoff['ID_shuffle']):
                    idx_train = [df_cutoff.loc[_, 'index'] for _ in split_fold[0]]
                    idx_test = [df_cutoff.loc[_, 'index'] for _ in split_fold[1]]
                    split_.append(((idx_train, idx_test), features_to_include))

            split_ = [(idx, _, feat_) for idx, (_, feat_) in enumerate(split_)]
    ###############################
    ...
    if n_jobs in {0, 1}:
        for train_test_idx in split_:
            ...
            testing_error, training_error, splitter_df, splitter_list = fit_cv_fold(train_test_idx,
                                                                     df,
            ...
            if not (testing_error is None):
                ...
                splitter_df_list.append((splitter_df, splitter_list))
    ...
    # training using multiprocessing
        ...
        # save the training and testing error
        for testing_error, training_error, splitter_df, splitter_list in results:
            if not (testing_error is None):
                df_training_error = df_training_error.merge(training_error, how='outer', on=['Clusters'])
                df_testing_error = df_testing_error.merge(testing_error, how='outer', on=['Clusters'])
                splitter_df_list.append((splitter_df, splitter_list))
...
if __name__ == "__main__":
    from codes.data_utils import (load_data)
    from codes.params import validation_cutoff, region_exceptions_dict,target_col, region_col, date_col, mdp_features_dict
    import warnings
    warnings.filterwarnings("ignore")
    df, _, _ = load_data(validation_cutoff=validation_cutoff)
    n_days = 15

    createSamplesRelative(df,  # dataframe: original full dataframe
                  target_col,  # str: col name of target_colname (i.e. 'deaths')
                  region_col,  # str, col name of region (i.e. 'state')
                  date_col,  # str, col name of time (i.e. 'date')
                  mdp_features_dict[region_col][target_col],  # list of str: i.e. (['mobility', 'testing'])
                  ([], 0),  # int list: defining size of jumps in stringency
                  # d_delay, # int: day lag before calculating death impact
                  7,  # int: # of days to average when reporting death
                  region_exceptions=None)
